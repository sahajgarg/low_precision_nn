...
import models.dlrm.data_utils as data_utils
...
import models.dlrm.data_loader_terabyte
        ...
        if self.memory_map:
            if self.split == 'none' or self.split == 'train':
                ...
                if index == self.offset_per_file[self.day]:
                    '''
                    if not hasattr(self, 'day_boundary'):
                        diffs = [index - offset for offset in self.offset_per_file]
                        day, diff = None, None
                        for idx, val in enumerate(diffs):
                            if day is None or 0 <= val < diff:
                                day = idx
                                diff = val
                        self.day = day
                    '''
