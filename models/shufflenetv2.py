...
class QuantizableShuffleNetV2(shufflenetv2.ShuffleNetV2):
    def __init__(self, *args, **kwargs):
        ...
        self.quant_globalpool = torch.quantization.QuantStub()
    ...
    def forward(self, x):
        ...
        # TODO: for bitwidth learning experiments, this will be ignored and
        # kept at 8 bits because it is a quant stub, and that is typically used
        # for inputs. However, we only use this for noise models for now 
        x = self.quant_globalpool(x)
        x = self.fc(x)
        x = self.dequant(x)
        return x
